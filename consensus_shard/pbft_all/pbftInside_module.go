...
func (rphm *RawRelayPbftExtraHandleMod) HandleinPropose() (bool, *message.Request) {
	// new blocks
	block := rphm.pbftNode.CurChain.GenerateBlock(0)
	r := &message.Request{
...
func (rphm *RawRelayPbftExtraHandleMod) HandleinCommit(cmsg *message.Commit) bool {
	...
	if rphm.pbftNode.NodeID == rphm.pbftNode.view {
		...
		for _, tx := range block.Body {
			...
			// if this transaction is on chain for the first time...
			if clientID, ok := rphm.pbftNode.txRequest2Client[string(tx.TxHash)]; ok {
				isOn, bhash, txroot, bheight, keylist, valuelist := rphm.pbftNode.CurChain.TxOnChainVerify(tx.TxHash)
				rphm.pbftNode.SendProofMsg(bhash, txroot, string(tx.TxHash), clientID, bheight, isOn, keylist, valuelist)
				// delete to save memory
				delete(rphm.pbftNode.txRequest2Client, string(tx.TxHash))
			}
		}
		// send relay txs
