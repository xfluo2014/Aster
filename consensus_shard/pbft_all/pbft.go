...
type PbftConsensusNode struct {
	...
	// Transaction Requests from client.
	txRequest2Client map[string]string // txHash to clientIp
...
func NewPbftNode(shardID, nodeID uint64, pcc *params.ChainConfig, messageHandleType string) *PbftConsensusNode {
	...
	fp := "./expTest/record/ldb/s" + strconv.FormatUint(shardID, 10) + "/n" + strconv.FormatUint(nodeID, 10)
	var err error
	...
	p.txRequest2Client = make(map[string]string)
...
func (p *PbftConsensusNode) handleMessage(msg []byte) {
	...
	// About client
	case message.CClientTxSend:
		p.handleClientTxSend(content)
	case message.CClientRequestTxProof:
		p.handleClientRequestTxProof(content)
