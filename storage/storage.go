...
func NewStorage(cc *params.ChainConfig) *Storage {
	_, errStat := os.Stat("./expTest/record")
	if os.IsNotExist(errStat) {
		errMkdir := os.Mkdir("./expTest/record", os.ModePerm)
		if errMkdir != nil {
			log.Panic(errMkdir)
		}
	} else if errStat != nil {
	...
	s := &Storage{
		dbFilePath:            "./expTest/record/S" + strconv.FormatUint(cc.ShardID, 10) + "N" + strconv.FormatUint(cc.NodeID, 10) + "_database",
		blockBucket:           "block",
