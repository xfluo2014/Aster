...
func LightWeight_GenerateBatFile(nodenum, shardnum, modID int) {
	fileName := fmt.Sprintf("LW_bat_shardNum=%v_NodeNum=%v_mod=%v.bat", shardnum, nodenum, params.CommitteeMethod[modID])
	ofile, err := os.OpenFile(fileName, os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0777)
	if err != nil {
		log.Panic(err)
	}
	...
	for i := 0; i < nodenum; i++ {
		for j := 0; j < shardnum; j++ {
			str := fmt.Sprintf("start cmd /k go run main.go -l -n %d -N %d -s %d -S %d -m %d \n\n", i, nodenum, j, shardnum, modID)
			ofile.WriteString(str)
		}
	}
}
